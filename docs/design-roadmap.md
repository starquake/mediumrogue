...
| Q8 | Percentage stacking: add or compound? | **Conflict found:** #61 principle 14 mandates same-stat %s **add** (two +10% → +20%); the engine's `mulPct` fold **compounds** (×1.10·×1.10 = ×1.21, `rules.go`). Before skills ship as `mulPct` cards: add an additive-% fold phase, or carve out principle 14. Engine call with gameplay consequences. | ❓ |
| Q9 | Initial spawn: random scatter or bed/home default? | **Open fork on #36:** owner wants random per-player spawns (anti-pile-up); NGB1024 wants last-bed-visited as default and says piling is fine. Related: the bed/home-spawn future note and reconnect-at-your-bed. | ❓ |
| Q10 | Which skill model governs: plan §0 or the roadmap? | Plan §0 records as **decided** (2026-07-10): class-agnostic life skills in tiers I/II/III, level-up = one skill pick. Roadmap SK2/Q4 re-open it as ❓ with a **Class**/Adventure/Survival 3-tree + points model (#61). One of them should be marked superseded. | ❓ |
| Q11 | Subclass capstone-gating vs #61 principle 5? | #58/SU1 gate cross-class access on a class capstone — but principle 5 says progress in one tree may never block another. Reconcile: either subclass trees are exempt (they're not one of the 3 standard trees) or the gate needs a different key. | ❓ |
