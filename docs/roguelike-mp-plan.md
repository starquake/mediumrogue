...
9. **Shader filter pass:** the WebGL post-processing filter for the retro look. **(landed)** — CRT scanlines/vignette/desat+tint as a swappable `filter.ts` pass, default on, HUD toggle + persistence; bloom/flat-palette/flicker deferred.
