...
8. **Quests, parties & chat:** player quests and party quests, quest invites ("join my quest"), party membership, quest log UI, in-game chat panel (chat is core to the input-window social loop). The DOM social UI (chat, quest log, party roster, floating over the Pixi canvas) is built with **SolidJS** (a small, CSP-safe, native-TSX reactive-DOM library; see the §9 UI-toolkit decision).
