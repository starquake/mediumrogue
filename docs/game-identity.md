...
**Amended 2026-08-10 (#412): all three of those are TRAVEL behaviours.** Inside a combat bubble the cap is one entity per hex, and anything already stacked scatters when the fight starts. The blob, the random-member soak and the fit-on-one-tile limit describe getting somewhere, not fighting once you arrive — because the same paragraph read as a combat rule made stacking *strictly better* (the soak spreads damage a spread party concentrates, at no cost), and a strictly-better option is a default rather than a decision. Spatially this is a hex-grid tactics game, and a blob has no formation, no flanking and no chokepoint to hold.
