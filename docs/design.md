...
- **Melee:** walking into a hostile-occupied hex resolves as an attack — the attacker stays on their own hex and the move intent becomes an attack intent (the classic roguelike bump-to-attack), no special casing in the turn loop.
...
**Conflict resolution — DECIDED: phased. All attacks resolve first, then all moves.** *(Amended 2026-07-15, #104 — the original decision was moves-then-attacks; the flip and its reasoning live in `superpowers/specs/2026-07-15-attack-resolution-and-glancing-design.md`.)*
...
- **Melee sequencing:** a move intent whose next step is a hostile-held hex converts into an attack against that occupant (resolved in the attack phase, mover stays put). No post-move recheck — the target's pre-move position is what's hit, so the melee attack cannot fizzle by vacation.
