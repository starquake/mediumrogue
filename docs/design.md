...
- **Trigger on awareness (mutual LOS), not raw distance** — a distance-only trigger turns the stopped clock into a monster radar that leaks hidden enemies through walls. *(Implementation status: **shipped 2026-07-18** — #95. Bubbles and monster aggro both require line of sight: rock hard-blocks, forest costs `ForestSightCost` hexes of effective range each, water is transparent, endpoints don't count, symmetric. Ranged attacks were distance-only at first; **#195 (2026-07-20)** extended the same LOS gate to them — a shot through a wall now rejects at submit.)*
...
- Hex capacity is a config constant (`STACK_CAP`), **set to 5 — a full party fits on one hex.** Friendly stacking is on: allies share hexes, so a party can move as one blob (and share a click-to-move destination). The cap doubles as a soft mechanical nudge toward the ~3-parties-of-5 split — 15 players *can't* pile onto one hex. **Amended (2026-08-10, #412): this is the TRAVEL cap.** Inside a combat bubble it is 1, and an existing stack scatters as the fight forms — the blob is how you get somewhere, not how you fight once you arrive.
...
- **Melee** (decided 2026-07-15, #116) — three cases, no special casing in the turn loop:
  - **A player attacking an adjacent enemy**: the click (or key-step into the enemy's hex) sends an entity-targeted **attack intent** directly — no move intent exists at any point, so there is nothing to convert or stop. The classic roguelike bump *feel* survives purely in the client's input routing; attacking never moves the player.
  - **A player clicking a distant hostile-held hex**: that is just a **walk** — the server trims the route one hex short of the occupied destination (`queueMoveLocked`), so the player arrives adjacent and stops. Walking never auto-attacks; the attack is the player's next click.
  - **Monsters** keep true bump-to-attack: a monster's step onto a player-held hex resolves as an attack (the mover stays on its own hex). The conversion is monster-only.
...
- [x] ~~Tick rate~~ → **Decided: simultaneous 4 s turns (2 s input / ~2 s playback).** Keep the cadence a server config constant. **Feel-tested (playtest feedback batch 3, item 1, 2026-07-11):** the original 5 s/3 s split (`TurnSeconds`/`InputWindowSeconds`) felt slow in play — lowered to 4 s/2 s; `PlaybackSeconds` unchanged at 2 s. (`InputWindowSeconds` was removed in #464 — it enforced nothing and the server has never had a submission deadline; the decision above still stands, the constant recording it does not.)
...
- [x] ~~Ranged combat rules~~ → **Decided (6b.2):** bow & magic **range = 4 hexes**; **distance-only, no line-of-sight** requirement (terrain-blocked LOS shipped for bubbles/aggro in #95, but deliberately NOT for attacks); **no friendly fire** (ranged hits opposing faction only) — a bow into a mixed stack hits a random *hostile* member (the 6.3 random-member rule); mage magic is **AoE** (radius 1) hitting all hostiles in the area. Ranged resolves inside a combat bubble, simultaneous with melee. **Amended (item 7, playtest feedback batch 2):** a single-target shot (bow) now targets the victim's **entity id**, not a hex — the AoE cast stays ground-targeted (hex) since its blast radius makes a hex the natural target. Resolution re-aims at the entity-targeted victim's post-move hex, hitting if it is still in range from the shooter's own post-move hex and fizzling otherwise — this is the retreat-dodge rule (§5) finally applied honestly to ranged, since a hex-pinned target used to let a sidestepping victim dodge a shot it should have still eaten, and let a fleeing one still eat a shot it should have dodged. **Amended again (2026-07-15, #104):** with attacks now resolving before moves, the post-move re-aim goes away — every attack (bump, bow, AoE) resolves against pre-move positions and always lands; entity-targeting remains as the aiming UX for single-target shots. **Amended (2026-07-20, #195):** the "distance-only, no line-of-sight" carve-out is retired — a ranged shot now needs the same clear ray bubbles and aggro require (a rock between rejects at submit with `ErrNoLineOfSight`), and entity-targeted resolution is domain-guarded so a shot can't reach a victim frozen in another bubble.
...
- [ ] Combat math — mostly decided since: damage reduction is a **percentage** on gear (`take-damage ×0.9/×0.8` — converted from flat `−N` in #154 because subtractive mitigation stacked into the ≥1 clamp), with the dwarf species passive deliberately staying flat −1; player AoE **never hits allies** (no friendly fire, shipped); the general crit model is **`crit%` ×2, decoupled ARPG** (#69, `design-decisions.md`). Still open: weapon-variety base-damage spread and AoE templates beyond blast (line/cone)
- [x] ~~HP/resource recovery~~ → **Direction decided (2026-07-10), layered** (the parenthetical below described 2026-07-10, when there was NO healing at all and death was literally the only way to refill HP — the inverted incentive this list exists to fix. Layers 1 and 2 have since shipped; see FEATURES.md for what actually exists):
  1. **Passive out-of-combat regen** (first slice, small): X HP per world turn while not in a bubble, tuned slow — kills the death-as-heal exploit without devaluing everything below. Knob. **Amended (2026-07-28, #322 decision 11): regen applies IN a bubble too**, not only out of one — so "out-of-combat regen" is no longer the shipped shape, and both HP and energy tick on every turn resolution in either domain. `RegenPerTurn` stays at **1** precisely because of that: a trickle that small cannot outpace a monster hitting for 3-9, so mid-fight recovery never becomes attrition removed rather than managed. *(The original wording above survived in three places long after it stopped being true — corrected in #398.)*
  2. **Potions as consumable items** (drops → inventory → drink): drinking follows the equip rule — **free outside a bubble, your whole turn inside** — so spending a combat turn drinking instead of swinging is a real choice (never auto-drink; the choice IS the mechanic). UI offers two spends because "efficient" differs by context: **top-up** (waste-optimal — missing 8 HP, drink one 5-bottle) out of combat vs **heal-me** (turn-optimal — biggest bottle first, spill the excess) in combat, each previewing waste. **Amended (2026-08-10, #410/#415): the flat heal ladder is deleted and so is the machinery behind it**, so none of the spend UI above describes anything shipped — passive regen plus the free `R` draught made a flat bottle strictly worse the higher your level went. A recovery consumable, if one returns, is a timed `regen` effect (HP over turns, the Second Wind shape), which has no "biggest bottle first" question to answer.
