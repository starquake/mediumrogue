...
test("both globes render, and E drinks the energy draught", async ({ page }) => {
  ...
  // Ready: the corner shows a tick rather than a number, so the shape does not
  // jump when a cooldown appears.
  await expect.poll(() => page.locator("#globe-energy .cd").textContent()).toBe("✓");
  ...
  await expect.poll(() => page.locator("#globe-energy .cd").textContent()).not.toBe("✓");
